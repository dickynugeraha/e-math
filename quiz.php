    ...
    submitBtn.addEventListener("click", () => {
      ...
      if (answer) {
        ...
        if (currentQuiz < quizzes.length) {
          ...
          $.ajax({
            url: "tambah-nilai.php",
            type: "POST",
            data: {
              userId: userId,
              nilai: score / quizzes.length,
              type: "pilihan_ganda"
            },
            success: function(response) {
              alert("Nilai berhasil direkam!");
            },
            error: function(jqXHR, textStatus, errorThrown) {
              alert("Nilai gagal direkam!");
          ...
          quiz.innerHTML = `
        <div style="padding: 3rem; padding-bottom: 4rem;">
          <h5 class='text-center'>Kamu mendapatkan nilai <i>${score/quizzes.length}/100</i>, dari 10 soal yang sudah dikerjakan</h5>
        </div>
